...
Scala 语言没有 Rank N Type支持, 但是 [[*/{{{ruby(自然变换,Natural Transformations)}}}/][{{{ruby(自然变换,Natural Transformations)}}}]] 提到过可以用 =apply= 来模拟. 其实可以使用 Cats 的 [[https://typelevel.org/cats/datatypes/functionk.html][FunctionK(~>)]] 更方便:
#+BEGIN_SRC scala
